...
def sort(input, dim=-1, descending=False, out=None):
    ...
    if dim_size_padded != dim_size:
        ...
        out_padded = torch.empty(out_padded_shape, dtype=input.dtype, device=input.device)

        input_padded = torch.empty(out_padded_shape, dtype=input.dtype, device=input.device)
