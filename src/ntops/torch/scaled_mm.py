...
import ntops
from ntops.torch.scaled_grouped_mm import ScalingType, SwizzleType
...
def _validate_options(
    ...
    if not _enum_matches(scale_recipe_a, ScalingType, "BlockWise1x128"):
        raise NotImplementedError(
            "scale_recipe_a must be ScalingType.BlockWise1x128"
        )
    if not (
    ...
    for name, value in (("swizzle_a", swizzle_a), ("swizzle_b", swizzle_b)):
        if value is not None and not _enum_matches(
            value, SwizzleType, "NO_SWIZZLE"
        ):
...
def _validate_inputs(mat_a, mat_b, scale_a, scale_b, scale_recipe_b, bias):
    ...
    if (
        mat_a.dtype != _SUPPORTED_INPUT_DTYPE
        or mat_b.dtype != _SUPPORTED_INPUT_DTYPE
    ):
    ...
    if not mat_b.t().is_contiguous():
        raise ValueError(
            "mat_b must be column-major (a transposed contiguous matrix)"
        )
...
def _group_views(
    ...
    if b_uses_1x128:
        scale_b_groups = scale_b.reshape(
            n_blocks, _SCALE_BLOCK_SIZE, k_blocks
        ).permute(0, 2, 1)
    else:
        scale_b_groups = scale_b[:k_blocks].t().unsqueeze(-1)
        scale_b_groups = scale_b_groups.expand(-1, -1, _SCALE_BLOCK_SIZE)

    output_groups = output.view(
        m, n_blocks, _SCALE_BLOCK_SIZE
    ).permute(1, 0, 2)
    bias_groups = None
    if bias is not None:
...
def scaled_mm(
    ...
    """Block-scaled FP8 matrix multiplication for inference projections."""
